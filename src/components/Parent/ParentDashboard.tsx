...
import {
  ...
  PlanTag,
  LockedFeature,
} from './ParentDashboard/components';
import {
  describeProgressStatus,
  deriveSessionLengthPreference,
  formatCheckInTimeAgo,
  checkInBadgeTone,
  describeActivityType,
  ELECTIVE_SUBJECT_OPTIONS,
  type GoalFormState,
} from './ParentDashboard/utils';
import {
  weeklyPlanToneStyles,
} from './ParentDashboard/utils/styles';
...
const ParentDashboard: React.FC = () => {
    ...
    () => {
      ...
      if (primaryGap) {
        cards.push({
          ...
          detail:
            primaryGap.summary ??
            `Next focus: ${primaryGap.concepts?.[0] ?? formatSubjectLabel(primaryGap.subject)}${gapMastery != null ? ` (${gapMastery}% accuracy)` : ''
            }`,
  ...
  const handleShareHomeExtension = async (
    ...
    const shareText = `At-home focus: ${focusLabel} for ${learnerName}. Try "${activity.title}"${activity.estimatedMinutes ? ` (~${activity.estimatedMinutes} min)` : ''
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {/* Phase 6: Parent Summary Card - "Summary First" view */}
          {dashboard?.children && (
            <motion.div
            ...
            >
              <ParentSummaryCard
                ...
                onViewDetails={() => scrollToSection('child-insights')}
                onSetGoals={() => scrollToSection('goal-planner')}
              />
            </motion.div>
          )}

          {/* Phase 6: Path Explanation + Coaching Suggestions for selected child */}
          {currentChild && (
            <motion.div
              initial={{ opacity: 0, y: 12 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <PathExplanationCard
                ...
                diagnosticStatus={currentChild.diagnosticStatus}
                diagnosticCompletedAt={currentChild.diagnosticCompletedAt}
              ...
              <WeeklyCoachingSuggestions
                childName={currentChild.name}
                suggestions={currentChild.coachingSuggestions ?? []}
                focusSubject={currentChild.learningPreferences?.focusSubject !== 'balanced' ? currentChild.learningPreferences?.focusSubject : null}
                        ...
                        <div className="flex items-center justify-between text-[11px] text-slate-700 gap-2">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              ...
                              className={`inline-flex items-center gap-1 rounded-lg px-2 py-1 font-semibold border ${reminderEnabled
                                ? 'bg-slate-100 text-slate-700 border-slate-200'
                        ...
                        {child.alerts.length > 0 && (
                          <div className="flex items-center justify-between text-[11px] text-slate-700">
                            ...
                            <div className="flex gap-1">
                              <span
                                className={`px-2 py-1 rounded-full border ${followUpEntry.assignment
                              ...
                              <span
                                className={`px-2 py-1 rounded-full border ${followUpEntry.diagnostic
                                  ? 'bg-emerald-50 border-emerald-200 text-emerald-700'
                                  : 'bg-white border-slate-200 text-slate-600'
                                  }`}
                              >
                              ...
                              <span
                                className={`px-2 py-1 rounded-full border ${followUpEntry.resolved
                                  ? 'bg-emerald-50 border-emerald-200 text-emerald-700'
                                  : 'bg-white border-slate-200 text-slate-600'
                                  }`}
                              >
                      ...
                      <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
                        {sortedPlans.map((plan) => {
                          ...
                          return (
                            <div
                              key={plan.slug}
                              className={`rounded-xl border p-3 space-y-2 ${isCurrent ? 'border-brand-teal bg-brand-light-teal/40' : 'border-slate-200 bg-white'
                                }`}
                            >
                              ...
                              <button
                                ...
                                className={`inline-flex w-full items-center justify-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold focus-ring ${isCurrent
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-[11px] px-2 py-1 rounded-full ${learnerStepDone ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                          }`}
                      >
                    ...
                    <p className="text-[11px] text-slate-600">
                      {seatLimit !== null
                        ? `Seats used: ${seatsUsed}/${seatLimit}. ${seatLimitReached
                          ? 'Upgrade to add another learner.'
                          : `${seatsRemaining} seat${seatsRemaining === 1 ? '' : 's'} remaining.`
                        }`
                        : billingRequired
                  ...
                  <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-[11px] px-2 py-1 rounded-full ${hasGoalsSet ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                          }`}
                      >
                  ...
                  <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-[11px] px-2 py-1 rounded-full ${diagnosticStepDone ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                          }`}
                      >
            ...
            <div className="flex flex-wrap gap-4">
              ...
              {(dashboard?.children ?? []).map((child) => (
                <button
                  ...
                  className={`flex items-center space-x-3 px-6 py-4 rounded-2xl transition-all ${currentChild?.id === child.id
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-4 space-y-2">
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    <button
                      ...
                      className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 font-semibold ${todayLaneState.goal === 'done'
                        ? 'bg-emerald-100 text-emerald-700 border-emerald-200'
                    ...
                    <button
                      ...
                      className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 font-semibold ${todayLaneState.goal === 'skipped'
                        ? 'bg-amber-100 text-amber-700 border-amber-200'
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2 text-xs">
                        <button
                          ...
                          className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 font-semibold ${todayLaneState[`assignment-${nextAssignment.id}`] === 'done'
                            ? 'bg-emerald-100 text-emerald-700 border-emerald-200'
                            : 'bg-white text-slate-700 border-slate-200 hover:border-brand-blue/50'
                            }`}
                        >
                        ...
                        <button
                          ...
                          className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 font-semibold ${todayLaneState[`assignment-${nextAssignment.id}`] === 'skipped'
                            ? 'bg-amber-100 text-amber-700 border-amber-200'
                            : 'bg-white text-slate-700 border-slate-200 hover:border-amber-300/80'
                            }`}
                        >
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2 text-xs">
                        ...
                        <button
                          ...
                          className={`inline-flex items-center gap-1 rounded-lg border px-2 py-1 font-semibold ${todayLaneState[`adaptive-${recommendedModules[0].id}`] === 'skipped'
                            ? 'bg-amber-100 text-amber-700 border-amber-200'
                            : 'bg-white text-slate-700 border-slate-200 hover:border-amber-300/80'
                            }`}
                        >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-[11px] px-2 py-1 rounded-full ${coverageSummary.needMoreData
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center bg-slate-100 rounded-full p-1 text-xs">
                    <button
                      ...
                      className={`px-3 py-1 rounded-full font-semibold ${insightTab === 'overview' ? 'bg-white shadow-sm text-brand-violet' : 'text-gray-600'
                        }`}
                    >
                      Adaptive plan
                    </button>
                    <button
                      ...
                      className={`px-3 py-1 rounded-full font-semibold ${insightTab === 'skill_gaps' ? 'bg-white shadow-sm text-brand-violet' : 'text-gray-600'
                        }`}
                    >
                      Skill gaps
                    </button>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                                ...
                                <div className="rounded-lg border border-slate-200 bg-white p-3 space-y-2">
                                  ...
                                  <div className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                                    ...
                                    <button
                                      ...
                                      className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition ${allowTutorChats
                                        ? 'bg-emerald-100 text-emerald-700 border-emerald-200'
                                        : 'bg-slate-100 text-slate-700 border-slate-200'
                                        }`}
                                    >
                                  ...
                                  <div className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                                    ...
                                    <button
                                      ...
                                      className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition ${lessonContextOnly
                                    ...
                                    {(
                                      ...
                                      return (
                                        <button
                                          ...
                                          className={`px-3 py-2 rounded-lg text-sm font-semibold border transition ${active
                  ...
                  <div className="space-y-4 w-full">
                    <div className="rounded-xl border border-slate-200 p-4 space-y-3">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        >
                          <RefreshCw
                            className={`h-4 w-4 ${overviewFetching || concernReportsQuery.isFetching ? 'animate-spin' : ''
                              }`}
                          />
                        </button>
                      </div>
                          ...
                          {safetyEvents.map((event) => (
                            ...
                            >
                              <div className="flex items-start justify-between gap-3">
                                ...
                                <span
                                  className={`text-[11px] px-2 py-1 rounded-full ${event.severity === 'warn'
                                    ? 'bg-amber-100 text-amber-700'
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 text-[11px] text-slate-700 mb-2">
                  <span
                    className={`px-2 py-1 rounded-full border ${openPrivacyRequests.length
                        ...
                        privacyHistoryByChild.map((entry) => (
                          ...
                          >
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span
                                className={`text-[11px] px-2 py-1 rounded-full ${entry.pending
                                  ? 'bg-amber-100 text-amber-700'
                                  : entry.total
                                    ? 'bg-emerald-100 text-emerald-700'
                                    : 'bg-slate-100 text-slate-700'
                            ...
                            <p className="text-xs text-slate-600">
                              {entry.total
                                ? `${entry.total} request${entry.total === 1 ? '' : 's'} • Last ${entry.lastType === 'erasure' ? 'deletion' : 'export'
                  ...
                  {showSkeleton ? (
                    ...
                    sortedDashboardAlerts.map((alert) => {
                      ...
                      return (
                        <div
                          key={alert.id}
                          className={`p-4 rounded-xl border ${alert.type === 'warning'
