...
type RubricKey = 'answered' | 'evidence' | 'explained' | 'revised';

const BLOCK_LABELS: Record<string, string> = {
  warmup: 'Warmup',
  diagnostic: 'Starting check',
  lesson: 'Mini-lesson',
  guided_practice: 'Evidence response',
  independent_practice: 'Writing response',
  repair: 'Repair',
  challenge: 'Challenge',
...
const RUBRIC: Array<{ key: RubricKey; label: string; points: number }> = [
  { key: 'answered', label: 'Answered the prompt clearly', points: 25 },
  { key: 'evidence', label: 'Used text evidence or a concrete example', points: 30 },
  { key: 'explained', label: 'Explained how the evidence supports the answer', points: 25 },
  { key: 'revised', label: 'Checked grammar, wording, or organization', points: 20 },
];
...
const scoreResponse = (checks: Record<RubricKey, boolean>, response: string): number => {
  ...
  const lengthAdjustment = words >= 35 ? 0 : words >= 20 ? -5 : -15;
...
const ElaDailyBlockPage: React.FC = () => {
  ...
  const planQuery = useQuery({
    queryKey: ['ela-daily-block-plan', studentId],
  ...
  const subject = planQuery.data?.subjects.find((entry) => entry.subject === 'ela') ?? null;
  const contentQuery = useQuery({
    queryKey: ['ela-block-content', studentId, blockId],
    queryFn: () => fetchElaBlockContent(blockId ?? ''),
  ...
  const words = wordCount(response);
  const canSubmit = Boolean(block && studentId && !completed && words >= 12 && checks.answered);
  ...
  const handleSubmit = async () => {
    ...
    await emitStudentEvent({
      ...
      payload: {
        ...
        response_kind: prompt?.responseKind ?? 'ela_block_response',
        rubric_checks: checks,
        response_word_count: words,
        response,
        source: 'homeschool_ela_daily_plan',
      },
    });
    setCompleted(true);
  };
    ...
    return (
      <main className="min-h-screen bg-slate-50 px-4 py-8">
        <div className="mx-auto max-w-3xl rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-900">
          ...
          <h1 className="text-lg font-semibold">ELA block unavailable</h1>
        ...
        {planQuery.error && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-900">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h1 className="font-semibold">ELA block unavailable</h1>
                <p className="mt-1 text-sm text-amber-800">
                  This ELA plan item could not be loaded. Return to the dashboard and try another item.
                </p>
              </div>
            </div>
          </div>
        ...
        {contentQuery.error && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-900">
            <div className="flex items-start gap-3">
              <AlertTriangle className="mt-0.5 h-5 w-5 flex-shrink-0" />
              <div>
                <h1 className="font-semibold">ELA content unavailable</h1>
                <p className="mt-1 text-sm text-amber-800">
                  This block could not load its reading or mini-lesson. Return to the dashboard and try again.
                </p>
              </div>
            </div>
          </div>
        ...
        {!planQuery.isLoading && !planQuery.error && !block && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-900">
            <div className="flex items-start gap-3">
              <AlertTriangle className="mt-0.5 h-5 w-5 flex-shrink-0" />
              <div>
                <h1 className="font-semibold">ELA block not found</h1>
                <p className="mt-1 text-sm text-amber-800">The daily plan may have changed since this link was opened.</p>
              </div>
            </div>
          </div>
        )}
          ...
          <article className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="grid gap-0 lg:grid-cols-[1fr_320px]">
              <section className="p-6">
                ...
                <label htmlFor="ela-response" className="mt-6 block text-sm font-semibold text-slate-900">
                  Response
                </label>
                <textarea
                  id="ela-response"
                  ...
                  rows={12}
                  className="mt-2 w-full resize-y rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm leading-6 text-slate-900 shadow-sm outline-none transition focus:border-amber-500 focus:ring-2 focus:ring-amber-100"
                ...
                {completed && (
                  <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
                    Saved with a score of {score}%. The next ELA plan will use this evidence.
                  </div>
                )}
              </section>
              ...
              <aside className="border-t border-slate-100 bg-white p-6 lg:border-l lg:border-t-0">
                ...
                <button
                  ...
                  className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-full bg-amber-700 px-5 text-sm font-semibold text-white transition hover:bg-amber-800 disabled:cursor-not-allowed disabled:bg-slate-300"
                >
                  {isCompleting ? (
                  ...
                  {completed ? 'Saved' : 'Submit ELA evidence'}
                </button>
                ...
                {!canSubmit && !completed && (
                  <p className="mt-3 text-xs leading-5 text-slate-500">
                    Write at least 12 words and check the answer line before submitting.
                  </p>
                )}
              </aside>
            </div>
...
export default ElaDailyBlockPage;
