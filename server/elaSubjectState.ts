...
export type ElaCompletionEventInput = {
  moduleSlug: string;
  score: number;
...
type ElaSubjectStateBuildInput = ElaCompletionEventInput & {
  studentId: string;
  currentState?: StudentSubjectStateRow | null;
  elaMap: ElaModuleMap;
};

type ElaSubjectStateUpsert = {
  student_id: string;
  subject: 'ela';
...
const readElaModuleSlug = (payload: Record<string, unknown>): string | null => {
  for (const key of ELA_MODULE_SLUG_KEYS) {
    const slug = readString(payload[key]);
    if (slug) return slug;
  }

  const module = readRecord(payload.module);
  for (const key of ['slug', ...ELA_MODULE_SLUG_KEYS]) {
...
export const extractElaCompletionEvent = (
  payload: Record<string, unknown>,
  score: number | null | undefined,
  options: { accuracy?: number | null; completedAt?: string | null; timeSpentSeconds?: number | null } = {},
): ElaCompletionEventInput | null => {
  if (score == null || !Number.isFinite(score)) return null;

  const moduleSlug = readElaModuleSlug(payload);
  if (!moduleSlug) return null;

  const module = readRecord(payload.module);
  const subjectLooksEla = ELA_SUBJECT_KEYS.some((key) => isElaSubjectValue(payload[key]) || isElaSubjectValue(module[key]));
  const slugLooksEla = isElaSubjectValue(moduleSlug);
  if (!subjectLooksEla && !slugLooksEla) return null;
...
const readRecentEvidence = (metadata: Record<string, unknown>): ElaEvidence[] =>
  Array.isArray(metadata.recent_ela_evidence)
    ? metadata.recent_ela_evidence.flatMap((item) => {
...
const mapExistingState = (row: StudentSubjectStateRow | null | undefined): ElaSubjectState | null => {
  if (!row) return null;
...
const updateConfidence = (current: number | undefined, score: number): number => {
  ...
  if (score >= ELA_MASTERY_SCORE_THRESHOLD) {
    return clampConfidence(Math.max(base, 0.66) + 0.06);
  }
  if (score < ELA_WEAK_SCORE_THRESHOLD) {
...
const parentSummaryForOutcome = (title: string, score: number, outcome: 'mastered' | 'practice' | 'weak'): string => {
  if (outcome === 'mastered') {
    return `ELA marked ${title} mastered because the latest score was ${score}%.`;
  }
  if (outcome === 'weak') {
    return `ELA marked ${title} for repair because the latest score was ${score}%.`;
  }
  return `ELA kept ${title} in practice because the latest score was ${score}%.`;
};
...
export const buildElaSubjectStateUpdate = ({
  studentId,
  currentState = null,
  elaMap,
  moduleSlug,
  ...
  estimatedMinutes,
  payload,
}: ElaSubjectStateBuildInput): ElaSubjectStateUpsert | null => {
  const moduleEntry = elaMap.modules.find((entry) => entry.slug === moduleSlug);
  ...
  const completedEvidence: ElaEvidence = {
    moduleSlug,
  ...
  const outcome = normalizedScore >= ELA_MASTERY_SCORE_THRESHOLD
    ? 'mastered'
    : normalizedScore < ELA_WEAK_SCORE_THRESHOLD
      ? 'weak'
      : 'practice';
  ...
  const provisionalState: ElaSubjectState = {
  ...
  const decision = chooseElaAssignment({
    elaMap,
    state: provisionalState,
    recentEvidence: recentEvidenceForDecision,
    targetStrand: moduleEntry.strand,
  });
  const recommendedModule = elaMap.modules.find((entry) => entry.slug === decision.recommendedModuleSlug) ?? moduleEntry;
  const nextState: ElaSubjectState = {
    ...provisionalState,
  ...
  const nextMetadata = {
    ...
    mastered_module_slugs: masteredModuleSlugs,
    weak_module_slugs: weakModuleSlugs,
    recent_ela_evidence: recentEvidence.map((item) => ({
    ...
    last_ela_completion: lastCompletion,
...
export const updateElaSubjectStateFromCompletionEvent = async (
  supabase: SupabaseClient,
  studentId: string,
  event: ElaCompletionEventInput,
  options: { elaMap?: ElaModuleMap } = {},
): Promise<ElaSubjectStateUpsert | null> => {
  ...
  if (error) {
    throw new Error(`Unable to load ELA subject state: ${error.message}`);
  }
  ...
  if (result.error) {
    throw new Error(`Unable to update ELA subject state: ${result.error.message}`);
  }
