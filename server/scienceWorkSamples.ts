...
import type { ScienceAssignmentReason, ScienceModuleMap, ScienceStrand } from '../shared/scienceHomeschool.js';
import type { ScienceSubjectEvidenceSummary } from '../shared/scienceSubjectStateSummary.js';
import { loadScienceModuleMap } from './scienceHomeschoolPlans.js';

type ScienceSubjectStateUpdateLike = {
  metadata?: Record<string, unknown> | null;
};

export type StudentWorkSampleScienceRow = {
  module_slug: string;
...
const moduleTitle = (scienceMap: ScienceModuleMap, slug: string | null): string | null =>
  slug ? scienceMap.modules.find((module) => module.slug === slug)?.title ?? null : null;

const moduleStrand = (scienceMap: ScienceModuleMap, slug: string | null): ScienceStrand | null =>
  slug ? scienceMap.modules.find((module) => module.slug === slug)?.strand ?? null : null;

export const recordScienceWorkSampleFromSubjectStateUpdate = async (
  supabase: SupabaseClient,
  studentId: string,
  update: ScienceSubjectStateUpdateLike | null,
  options: {
    eventType: string;
  ...
  const lastCompletion = readRecord(update?.metadata?.last_science_completion);
  ...
  const insertPayload = {
    ...
    module_title: readString(lastCompletion.module_title) ?? moduleTitle(scienceMap, moduleSlug),
    strand: readString(lastCompletion.strand) ?? moduleStrand(scienceMap, moduleSlug),
    ...
    next_module_title: moduleTitle(scienceMap, nextModuleSlug),
    ...
    metadata: {
      source: 'science_subject_state_update',
  ...
  if (error) {
    if (isMissingWorkSamplesTable(error)) {
      console.warn('[science-work-samples] student_work_samples table is not available; skipping durable work sample write.');
      return false;
    }
    throw new Error(`Unable to record science work sample: ${error.message}`);
  }

  return true;
};

export const fetchScienceWorkSampleRows = async (
  supabase: SupabaseClient,
  studentId: string,
  options: { limit?: number } = {},
): Promise<StudentWorkSampleScienceRow[]> => {
  ...
  if (result.error) {
    if (isMissingWorkSamplesTable(result.error)) {
      console.warn('[science-work-samples] student_work_samples table is not available; falling back to science metadata.');
      return [];
    }
    throw new Error(`Failed to load science work samples: ${result.error.message}`);
  }

  return (result.data ?? []) as StudentWorkSampleScienceRow[];
};

export const scienceWorkSampleRowsToEvidence = (
  rows: StudentWorkSampleScienceRow[],
): ScienceSubjectEvidenceSummary[] =>
  rows.flatMap((row) => {
    ...
    return [
      {
        ...
        strand: (readString(row.strand) as ScienceStrand | null) ?? undefined,
        scorePct,
        completedAt: readString(row.completed_at) ?? undefined,
        estimatedMinutes: readNumber(row.estimated_minutes),
        outcome: (readString(row.outcome) as ScienceSubjectEvidenceSummary['outcome']) ?? undefined,
        reasonCode: (readString(row.reason_code) as ScienceAssignmentReason | null) ?? undefined,
