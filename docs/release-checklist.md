...
- Run `npm run eval:question-bank-gate` after any content import/seed wave and before release sign-off. This must pass with zero flagged, blocked, or generic question-bank rows.
...
- Live release-gates pass: run `npm run eval:release-gates:live -- --lookback-days 7` and attach the output; this must pass in live telemetry mode for launch sign-off.
- Full release content gate pass: run `npm run eval:release-gates:full` and attach the output. This combines live release gates with the question-bank quality gate.
- Synthetic release-gate runs are non-blocking rehearsal only: `npm run eval:release-gates:synthetic -- --lookback-days 7 --allow-missing-hard-gates` can validate no-user environments, but cannot be used as final launch evidence.
...
### Sign-off
- Link the Coverage Audits artifact (including `release_gates_live.txt`), QA notes, and gate results to the release PR/ticket. Capture who approved each gate (content, QA, pedagogy).
