...
async function rebuildPath() {
    ...
    const { data: modules, error: modError } = await supabase
        .from('modules')
        .select('id, title, subject, grade_band, slug')
        .in('grade_band', ['6', '7', '8'])
    ...
    pathModules.forEach((m, i) => console.log(`  ${i + 1}. ${m.title} (${m.subject})`));

    // Insert path entries with type 'lesson' (as expected by the DB enum)
    const entries = pathModules.map((m, i) => ({
        path_id: PATH_ID,
        position: i + 1,
        type: 'lesson', // This is the correct enum value
        module_id: m.id,
        lesson_id: null,
        assessment_id: null,
        status: 'not_started',
    ...
    if (!apply) {
        console.log(`Would insert ${entries.length} lesson-typed path entries into path ${PATH_ID}.`);
        return;
    }
