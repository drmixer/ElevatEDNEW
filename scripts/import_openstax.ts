...
const main = async () => {
  ...
  logWriteMode(apply, 'OpenStax assets');
  const mapping = (await loadStructuredFile<OpenStaxMapping>(file)) ?? {};

  const supabase = createServiceRoleClient();
  const inserted = await importOpenStaxMapping(supabase, mapping, apply);
  console.log(apply ? `Upserted ${inserted} OpenStax assets from ${file}` : `Would upsert ${inserted} OpenStax assets from ${file}`);
};
