-- 020_phase14_activities_projects.sql
-- Adds authored content source for internally generated activities/projects and expands coverage views
-- to track activities/projects separately from generic external links.

begin;

insert into public.content_sources (name, license, license_url, attribution_text, metadata)
values
  (
    'ElevatED Authoring',
    'CC BY',
    'https://creativecommons.org/licenses/by/4.0/',
    'ElevatED Authoring (CC BY)',
    jsonb_build_object('notes', 'Internal activity and project assets generated for Phase 14')
  )
...
-- Recreate coverage views to expand activity/project tracking.
drop view if exists public.coverage_dashboard_rollup;
drop view if exists public.coverage_dashboard_cells;
...
asset_counts as (
  select
    ...
    count(distinct a.id) filter (
      where (
        coalesce((a.metadata->>'storage_mode')::text, 'stored') in ('link', 'embed')
      )
      and lower(coalesce(a.kind, '')) not in ('activity', 'project')
      and coalesce((a.metadata->>'activity_type')::text, '') not in ('project', 'teacher_led', 'independent', 'reflection')
    ) as external_resource_count,
    count(distinct a.id) filter (
      where lower(coalesce(a.kind, '')) = 'activity'
        or coalesce((a.metadata->>'activity_type')::text, '') in ('teacher_led', 'independent', 'reflection')
    ) as activity_count,
    count(distinct a.id) filter (
      where lower(coalesce(a.kind, '')) = 'project'
        or coalesce((a.metadata->>'activity_type')::text, '') = 'project'
    ) as project_count,
    count(distinct a.id) filter (
      where coalesce((a.metadata->>'home_extension')::boolean, false) = true
    ) as home_extension_count,
...
module_summary as (
  select
    ...
    coalesce(ac.project_count, 0) as project_count,
    coalesce(ac.home_extension_count, 0) as home_extension_count,
...
select
  ...
  coalesce(ms.activity_count, 0) > 0 as meets_activity_baseline,
  coalesce(ms.project_count, 0) > 0 as meets_project_baseline,
  greatest(
...
with module_cells as (
  select
    ...
    bool_and(meets_activity_baseline) as meets_activity_baseline,
    bool_and(meets_project_baseline) as meets_project_baseline
...
select
  ...
  count(*) filter (where meets_activity_baseline) as modules_with_activities,
  count(*) filter (where meets_project_baseline) as modules_with_projects,
  count(*) filter (
    where not (
